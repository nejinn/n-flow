...
func MdErrorResult(r *http.Request, w http.ResponseWriter, msg string) {
	errMsg := fmt.Sprintf("%s ,%s", xerr.MapErrMsg(xerr.MD_COMMON_ERROR), msg)
	httpx.WriteJson(w, http.StatusOK, Error(xerr.MD_COMMON_ERROR, errMsg))
}
...
func MapErrorResult(r *http.Request, w http.ResponseWriter, code uint32, msg string) {
	...
	httpx.WriteJson(w, http.StatusOK, Error(code, errMsg))
}
